...
class SortMessagesByRecipientIDEndpoint(BaseEndpoint):

    async def method_get(self, request: Request, body: dict, session: DBSession, recipient_id: int,
                         token: dict, *args, **kwargs) -> BaseHTTPResponse:

        db_message = message_queries.get_messages_by_recipient_id(session, token.get('eid'), recipient_id)
        try:
