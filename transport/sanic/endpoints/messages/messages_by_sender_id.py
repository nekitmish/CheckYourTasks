...
class SortMessagesBySenderIDEndpoint(BaseEndpoint):

    async def method_get(self, request: Request, body: dict, session: DBSession, sender_id: int,
                         token: dict, *args, **kwargs) -> BaseHTTPResponse:

        db_message = message_queries.get_messages_by_sender_id(session, token.get('eid'), sender_id)
        try:
