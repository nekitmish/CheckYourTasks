...
class GetMyEmployeeByLoginEndpoint(BaseEndpoint):

    async def method_get(self, request: Request, body: dict, session: DBSession, login: str, token: dict, *args, **kwargs
                         ) -> BaseHTTPResponse:

        eid = session.get_employee_id_by_login(login)
